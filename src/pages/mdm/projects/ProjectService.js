...
export const useService=()=>{
    ...
    useEffect(()=>{
Getdata('http://localhost:8186/project').then(d=>setData(d?.projects||[]))
    },[])
   
    const add=(v)=>new Promise((resolve,reject)=>{
        fetch("http://localhost:8186/project",{
            method:"POST",
            headers:
            { 
    ...
    const update=(v)=>new Promise((resolve,reject)=>{
        fetch("http://localhost:8186/project",{
            ...
            body: JSON.stringify({project:{...v,createdOn:null,modifiedOn:null}})}).then(d=>{
            if(d.status<300){
    ...
    const deletec=(v)=>{
        ...
        Deletedata("http://localhost:8186/project","DELETE",v.projectCode).then(d=>{
